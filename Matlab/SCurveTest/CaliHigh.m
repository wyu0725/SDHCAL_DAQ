...
prompt = {'Input ASIC ID','Input ASIC Index'};
dlg = 'Input data info';
answer = inputdlg(prompt,dlg);
AsicID = str2double(answer(1));
AsicIndex = str2double(answer(2));
DacRange = 101;
...
for i = 1:1:PackageNumberHigh
    if(i<=10)
        filename = sprintf('%s\\ASIC%dSCTest%dfCHighGain.dat',filepath,AsicIndex,ChargeHigh(i));
    else
...
for i = 1:1:PackageNumberHigh
    Dac0Value(i) = Dac0FitP(i,3,2);
    Dac0Sigma(i) = 1/(sqrt(2)*Dac0FitP(i,2,1));
    Dac1Value(i) = Dac1FitP(i,3,2);
    Dac1Sigma(i) = 1/(sqrt(2)*Dac1FitP(i,2,1));
end
figure;
